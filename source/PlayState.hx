...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['Your bad at the game', 0.2], //From 0% to 19%
		['Garbage', 0.4], //From 20% to 39%
		['Very bad', 0.5], //From 40% to 49%
		['Not bad!', 0.6], //From 50% to 59%
		['Pretty fine', 0.69], //From 60% to 68%
		['Gettin better', 0.7], //69%
		['Keep Going!', 0.8], //From 70% to 79%
		['KEEP GOING!!', 0.9], //From 80% to 89%
		['Your the best!', 1], //From 90% to 99%
		['Uour a master!!!', 1] //The value on this one isn't used actually, since Perfect is always "1"
	]; 
		...
		if (!note.wasGoodHit)
		{
			switch(note.noteType) {
				case 3:
					...
					if(!boyfriend.stunned)
					{
						boyfriend.playAnim('boyfriend dodge', true);
						boyfriend.specialAnim = true;
						if(!endingSong)
						{
							RecalculateRating();
